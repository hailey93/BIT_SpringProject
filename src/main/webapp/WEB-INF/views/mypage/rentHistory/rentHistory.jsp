	...
	<div id="divContentsW">
		...
		<div id="divContents">
				...
				<div>
					<div id="mTS_2" class="mTSWrapper mTS_horizontal">
						<ul class="mTSContainer" id="mTS_2_container"
							...
							<li class="selected mTSThumbContainer"><a
								href="rentHistory.do">대여이력</a></li>
				...
				<table class="mobileTable tablet">
					...
					<thead>
						<tr>
							...
							<th scope="row" data-hide="phone" class="footable-last-column"
								style="display: table-cell;">반납일</th>

						</tr>
					</thead>
					<tbody>

						<c:forEach items="${rentHistoryList}" var="rent"
							varStatus="status">
							...
							<tr>
								...
								<td class="bookTitle"><a
									href="/lib/bookDetail.do?bookCode=${rent.bookCode}">${rent.bookTitle}</a></td>
			...
			<div class="paging">
				...
				<span style="float: inherit;"> <c:if
						test="${paging.prev == true }">
						<a href="/lib/rentHistory.do?page=1" class="page"> <img
							src="webimg/firstPage.gif">
						</a>
					</c:if> <c:if test="${paging.prev == true }">
						<a
							href="/lib/rentHistory.do?page=${paging.pageSize}&range=${paging.range-paging.pageSize}"
							class="page"> <img src="webimg/prevPage.gif">
						</a>
					</c:if> <c:forEach begin="1" end="${paging.pageSize }" varStatus="page">

						<c:choose>

							<c:when
								test="${paging.page+paging.range == page.count+paging.range}">
								<span>${paging.page+paging.range}</span>
							</c:when>

							<c:when test="${page.count+paging.range <= paging.pageCnt}">
								<a
									href="/lib/rentHistory.do?page=${page.count}&range=${paging.range}">
					...
					</c:forEach> <c:if test="${paging.next == true }">
						<a
							href="/lib/rentHistory.do?page=1&range=${paging.range+paging.pageSize}"
							class="page"> <img src="webimg/nextPage.gif">
						</a>
					</c:if> <c:if test="${paging.next == true }">
						<a
							href="/lib/rentHistory.do?page=${paging.pageCnt%paging.pageSize }
															&range=${paging.pageCnt-paging.pageCnt%paging.pageSize}"
							class="page"> <img src="webimg/lastPage.gif">
						</a>
					</c:if>
